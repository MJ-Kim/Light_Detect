    ...
    def detect_objects(self, frame, output_path,filename):
        ...
        for i in range(0,20) :
            if (self.output_dict['detection_scores'][i]>0.25):
                ...
                sum_val=0
                for i in range(wid):
                    for j in range(5, hei-5, 5):
                        if hsv[j, i, 2]!= 0 :
                            sum_hue = sum_hue + (hsv[j, i, 0])
                            sum_val= sum_val+(hsv[j,i,2])
                            # (j=y, i=x) 순서주의  add Hue value
                            counter = counter + 1
                hue = int(sum_hue/counter)
                val = int(sum_val/counter)
                c_hsv=np.uint8([[[hue/2,255,val]]])
                c_bgr=cv2.cvtColor(c_hsv, cv2.COLOR_HSV2BGR)
                c=etree.SubElement(ob,"color")
                b=etree.SubElement(c,"b")
                b.text=str(c_bgr[0,0,0])
                g=etree.SubElement(c,"g")
                g.text=str(c_bgr[0,0,1])
                r=etree.SubElement(c,"r")
                r.text=str(c_bgr[0,0,2])
